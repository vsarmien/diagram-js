...
function BendpointMove(injector, eventBus, canvas, dragging, graphicsFactory, rules, modeling) {
  ...
  eventBus.on('bendpoint.move.move', function(e) {
    ...
    if (e.waypoints) {
      connection.waypoints = e.waypoints;
    } else {
  ...
  eventBus.on('bendpoint.move.end', function(e) {
    ...
    // handle waypoints layouted during snapping
    if (context.updateWaypoints) {
      dockingOrPoints = filterRedundantWaypoints(waypoints);
    } else {
      dockingOrPoints = waypoints[bendpointIndex];

      // ensure we have actual pixel values bendpoint
      // coordinates (important when zoom level was > 1 during move)
      dockingOrPoints.x = round(dockingOrPoints.x);
      dockingOrPoints.y = round(dockingOrPoints.y);
    }

    if (allowed && context.type === COMMAND_RECONNECT_START) {
      modeling.reconnectStart(context.connection, context.target, dockingOrPoints);
    } else
    if (allowed && context.type === COMMAND_RECONNECT_END) {
      modeling.reconnectEnd(context.connection, context.target, dockingOrPoints);
    } else
