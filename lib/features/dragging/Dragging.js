 ...
 *
 * Calling {@link Dragging#init} initializes a new drag
...
function Dragging(eventBus, canvas, selection) {
  ...
  function move(event, activate) {
    ...
    var payload = context.payload,
        ...
        globalCurrent = Event.toPoint(event) || { x: 0, y: 0 },
    ...
    // do not move without event
    if (!event) {
      return;
    }

    // prevent click and other event interaction
    suppressEvent(event);
