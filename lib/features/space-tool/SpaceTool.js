...
/**
 * A tool that allows users to create and remove
 * space in a diagram.
 *
...
function SpaceTool(eventBus, dragging, canvas, modeling, rules) {
  ...
  Tool.call(this, eventBus, { prefix: 'space', cursor: SPACE_TOOL_CURSOR });
  ...
  eventBus.on('space.selection.end', function(event) {
    ...
    eventBus.once('space.selection.ended', function() {
      self.activateMakeSpace(event.originalEvent);
    });
  });


  eventBus.on('space.move', HIGH_PRIORITY , function(event) {
  ...
  eventBus.on('space.end', function(event) {
...
SpaceTool.prototype.activateSelection = function(event, autoActivate) {
  this._dragging.init(event, 'space.selection', {
    trapClick: false,
...
SpaceTool.prototype.activateMakeSpace = function(event) {
  this._dragging.init(event, 'space', {
    autoActivate: true,
    cursor: SPACE_TOOL_CURSOR,
    data: {
      context: {}
...
SpaceTool.prototype.toggle = function(event) {
  if (this.isActive()) {
    this._dragging.cancel();
  } else {
    this.activateSelection(event);
  }
};
...
SpaceTool.prototype.isActive = function() {
  ...
  return context && /^space/.test(context.prefix);
};
